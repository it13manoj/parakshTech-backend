...
class TestimonialController extends Testimonial {

    async createbanner(req, res) {
        if (req.file.filename) {
            req.body.icon = req.file.filename;
        }
        await Testimonial.create(req.body).then(results => {
            if (results) {
    ...
    async TestimonialRecords(req, res) {
        try {
            ...
            if (result) {
                res.status(200).send(
                    Message.RESPONSEDATA({ MESSAGE: "Testimonial page records", RESULTS: result })
                );
            } else {
                res.status(404).send(
                    Message.ERROR.NOT_FOUND || { message: "No Testimonial page record found." }
                );
            }
        } catch (error) {
            console.error(error);
