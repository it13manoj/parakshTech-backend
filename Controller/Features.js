...
class FeatureController extends Feature {

    async createbanner(req, res) {
        if (req.file.filename) {
            req.body.icon = req.file.filename;
        }
        await Feature.create(req.body).then(results => {
            if (results) {
    ...
    async FeatureRecords(req, res) {
        try {
            ...
            if (result) {
                res.status(200).send(
                    Message.RESPONSEDATA({ MESSAGE: "Feature page records", RESULTS: result })
                );
            } else {
                res.status(404).send(
